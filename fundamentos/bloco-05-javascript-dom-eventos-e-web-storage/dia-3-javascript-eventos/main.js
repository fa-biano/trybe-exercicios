...
function clearClassTech () {
    let activeTech = document.querySelectorAll('.tech')
...
inputText.addEventListener('input', writeInputText);

function writeInputText() {
    let writeText = inputText.value;
    document.querySelector('.tech').innerText = writeText;
}
...
function openTop3List() {
    window.location.assign('https://fa-biano.github.io/');
}

let top3 = document.getElementById('my-spotrybefy');
top3.addEventListener('dblclick', openTop3List);

// 5. Crie uma função que, ao passar o mouse sobre 'Meu top 3 do Spotrybefy', altere a cor do mesmo;
function changeColor() {
    let color = 'rgb(' + (Math.random() * 250 + ',' + Math.random() * 250 + ',' + Math.random() * 250) + ')';
    top3.style.color = color;
}
top3.addEventListener('mouseover', changeColor);
