...
// const sendMarsTemperature = (reponseType) => {
//   const celsius = getMarsTemperature()
//   setTimeout(() => reponseType(celsius), messageDelay);
// }
...
// definição da função sendMarsTemperature...
const sendMarsTemperature = (reponseType, err) => {
  const msgHasSuccess = Math.round(Math.random() * 100)
  const celsius = getMarsTemperature();
  if (msgHasSuccess >= 40) {
    setTimeout(() => reponseType(celsius), messageDelay);
  } else {
